...
class PropertyHolder(type):
    def __new__(cls, name, bases, attrs):
        ...
        for attr in list(attrs) + sum(
            [list(vars(base)) for base in bases], []
        ):
...
class BaseModel(Model):
    ...
    async def __flush__(cls, target) -> None:
        ...
        keys = [
            MC_KEY_PAGINATE % (cls.__name__, p, PER_PAGE, count)
            for p in range(1, page_count + 1)
            for count in [True, False]
        ]
        keys.append(MC_KEY_ITEM_BY_ID % (target.__class__.__name__, target.id))
        await asyncio.gather(
            clear_mc(*keys), target.clear_mc(), return_exceptions=True  # noqa
        )
    ...
    @classmethod
    @cache(
        MC_KEY_PAGINATE % ('{cls.__name__}', '{page}', '{per_page}', '{count}')
    )
    async def paginate(
        cls, page: int = 1, per_page: int = PER_PAGE, count: bool = True
    ) -> Pagination:  # noqa
        if page < 1:
            page = 1
