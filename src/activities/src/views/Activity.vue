...
onMounted(() => {
    ...
    page = route.query.p || page.value
    getActivities(page).then(response => {
        activities.value = response.data.items.map(i => {
            return patchActivity(i)
        })
...
const toggleActivitySize = (activity) => {
...
const fetchCommentList = (activity) => {
...
const toggleShowComments = (activity) => {
...
const commentTo = (activity) => {
    if (!commentContent.value) {
        return
    }
    commentActivity(activity.id, `${replyTo} ${commentContent}`, refId).then(response => {
        if (response.data.r != 0) {
            toast.error(`评论失败: ${response.data.msg}`)
            if (response.data.r == 403) {
                toast(`登录中...`)
                window.location =  '/oauth/activities'
      }
...
const reply = (activity, comment) => {
  refId = comment.id
  replyTo = `回应 @${comment.user.username}`
}
</script>
